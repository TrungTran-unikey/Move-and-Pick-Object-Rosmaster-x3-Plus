...
class RosYoloDetector:
    ...
    def update_movement(self):
        ...
        with self.robot_lock:
            ...
            # For backward movement, apply real-time correction using model_box
            if self.movement_direction == -1 and self.use_box_model_correction:
                try:
                    # Get bounding box error from model_box predictions
                    dx_pixel, dy_pixel, total_error = self.get_bb_error()
                    
                    # Apply horizontal correction only (center to center alignment)
                    if abs(dx_pixel) > 3.0:  # 3 pixel threshold
                        # Calculate correction velocity (left/right only)
                        vy_correction = dx_pixel * 0.15  # Reduced gain for stability
                        
                        # Apply correction while maintaining backward movement
                        self.robot.move_with_bounding_box_correction(
                            vx_pixel=0,  # No forward/backward correction
                            vy_pixel=vy_correction,  # Horizontal correction only
                            speed=self.speed * 0.8,  # Reduced speed for precision
                            pixel_error=abs(dx_pixel),
                            tolerance_pixel=3.0
                        )
                        
                        rospy.logdebug(f"Backward movement correction: dx={dx_pixel:.1f}px, vy_correction={vy_correction:.2f}")
                except Exception as e:
                    rospy.logwarn(f"Error during backward movement correction: {e}")

            self.display_worker()  # Ensure display is ready

            # Call the robot's move_distance with current distance
    ...
    def get_bb_error(self):
        """
        Callback function for bounding box error - used by MecanumRobot API
        Returns: (dx_pixel, dy_pixel, total_error)
        """
        try:
            # Get latest frame data with results
            with self.frame_data_lock:
                if self.latest_frame_data is None:
                    return 0.0, 0.0, 999.0  # Large error if no data
            ...
            for result in results:
                if hasattr(result, 'boxes') and len(result.boxes) > 0:
                    ...
                    if target_detections:
                        # Get best detection
                        best_box, best_score = max(target_detections, key=lambda x: x[1])
                        ...
                        if self.use_box_model_correction:
                            # For model_box: only horizontal correction to image center
                            cx_target = self.IMG_ORG_W / 2  # Horizontal center of image
                            cy_target = cy_yolo  # Keep current Y position (no vertical correction)
                            
                            # Calculate error
                            dx = cx_target - cx_yolo  # Only horizontal error matters
                            dy = 0.0  # No vertical correction for model_box
                            total_error = abs(dx)  # Only horizontal error
                        else:
                            # For model_detect: full 2D correction to default bounding box center
                            cx_target, cy_target = self.default_bb_center
                            dx = cx_target - cx_yolo
                            dy = cy_target - cy_yolo
                            total_error = math.sqrt(dx*dx + dy*dy)
                        
                        return dx, dy, total_error
            
            # No target found
            return 0.0, 0.0, 999.0
            
        except Exception as e:
            print(f"BB error callback failed: {e}")
            return 0.0, 0.0, 999.0
            ...
            while not rospy.is_shutdown() and self.running:
                ...
                if not self.move_to_box:
                    ...
                    try:
                        ...
                        # Switch to box model for backward movement alignment
                        if self.switch_to_model("box"):
    ...
    try:
        ...
        if detector.move_gripper:
            ...
            try:
                ...
                detector.start_movement(1000, 180, detector.speed)  # Move back slightly
            ...
            detector.bot.set_motor(0, 0, 0, 0)  # Stop robot after movement
            rospy.sleep(2)
            # Move back to original position
            rospy.loginfo("Returning to original position...")
            detector.start_movement(distance-1000, 180, detector.speed)
